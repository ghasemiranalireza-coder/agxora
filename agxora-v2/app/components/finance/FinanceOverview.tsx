...
function MetricIcon({ id }: { readonly id: string }): JSX.Element {
  const paths: Record<string, string> = {
    ...
    "ai-score":
      "M12 3l2.2 4.5L19 9l-3.5 3.4L16.4 18 12 15.6 7.6 18l.9-5.6L5 9l4.8-1.5L12 3z",
  };
  return (
    <svg
      width="18"
...
function displayMetricValue(
  metric: FinanceOverviewMetric,
  t: (key: string, values?: Record<string, string | number>) => string,
): string {
  if (metric.id === "ai-score" && typeof metric.value === "string") {
    const match = /^(\d+)\s*\/\s*(\d+)$/.exec(metric.value.trim());
    if (match) {
      return t("finance.overview.scoreOutOf", {
        score: match[1],
        max: match[2],
      });
    }
  ...
  if (typeof metric.value === "number") {
    if (
      metric.currency &&
      metric.id !== "open-invoices" &&
      metric.id !== "paid-invoices"
    ) {
...
function displayMetricCaption(
  metric: FinanceOverviewMetric,
  t: (key: string, values?: Record<string, string | number>) => string,
): string {
  const caption = t(`finance.overview.metrics.${metric.id}.caption`);
  if (metric.captionAmount == null) return caption;
...
function displayDelta(
  ...
  return t(`finance.overview.metrics.${metric.id}.delta`, {
    days: metric.delta.days ?? "",
    count: metric.delta.count ?? "",
  });
}
            ...
            <FinanceGlassCard className="h-full" padding="p-5">
              <div className="flex items-start justify-between gap-3">
                <div className="flex items-center gap-3">
                  ...
                  >
                    {t(`finance.overview.metrics.${metric.id}.label`)}
                  </h3>
                </div>
